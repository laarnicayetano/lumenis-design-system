...
**Type.** ABC Arizona (Dinamo) throughout. Arizona **Sans Regular** is the workhorse; Arizona **Mix** appears _only_ to highlight 1–2 words. Titles: all caps, "as big as can be", leading 88–92% of the font size, tracking 0, optical kerning. Subtitle 20–40% of title size, leading 106–116%. Paragraph 50–65% of subtitle, leading 120–130%. Exactly one emphasis device per headline — Hero "L" _or_ Mix — never both, never repeated (p.27-29). Bold/Medium cuts are permitted only for high-contrast social pairings, never standalone.
...
**Surfaces, borders, radii.** The system is squared and flat: `--radius-none: 0` is the default everywhere, including CTA buttons and the box highlight. Cards are _not_ cards in the web sense — no fill, no shadow, no rounded corners; they are hairline rules (1px black) and generous whitespace, or an image with type beneath it. No elevation system exists; `--shadow-media` is the only shadow token and it is nearly invisible. Transparency and blur are not brand devices — for legibility over photography use a plain black scrim (`--scrim-image`), not a frosted capsule.
