...
**Sub-brand scoping** — the whole page sits inside `<div data-subbrand="optilight">` (`App.tsx`), so `--accent`/`--accent-contrast` resolve to OptiLIGHT Blue everywhere `variant="accent"` is used, per `tokens/subbrands.css`.

**Kit-local, not promoted to shared components**: the FAQ accordion (`Sectionsb.tsx`) and the booking modal (`Footer.tsx`) are one-off implementations local to this kit, not new design-system components — this repo doesn't have a generic `Accordion` or `Modal` yet. If a second use case needs either, that's the trigger to promote them, following the same reasoning that got `Rays` promoted.
