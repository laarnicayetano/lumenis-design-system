import {
  SplitLayout, SplitPanel, Eyebrow, Headline, Prose, Button,
  StatBlock, ProductCard, Icon, Quote, InsightCard, NewsletterSignup,
} from '../../components';
    ...
    <div>
      <SplitLayout ratio="6fr 6fr" minHeight="560px">
        <SplitPanel align="space-between" pad="var(--space-9) var(--page-gutter)">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
            ...
            <Prose size="subtitle" style={{ maxWidth: '30ch' }}>Minimally invasive, energy-based solutions for the Aesthetic and Vision markets.</Prose>
            <div style={{ display: 'flex', gap: 'var(--space-4)', marginTop: 'var(--space-4)' }}>
              <Button onClick={() => onOpenProduct('stellar-m22')}>Discover more</Button>
              <Button variant="secondary" onClick={() => onNavigate('contact')}>Talk to us</Button>
            </div>
          </div>
        </SplitPanel>
        <SplitPanel tone="image" pad="0"><ImagePlate label="Hero photography — practitioner" ratio="auto" style={{ height: '100%' }} /></SplitPanel>
      </SplitLayout>

      <Section tone="inverse" pad="var(--space-9) var(--page-gutter)">
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr) 1.4fr', gap: 'var(--space-8)', alignItems: 'start' }}>
          ...
          <Prose size="small" style={{ opacity: 0.8 }}>Lumenis develops life-changing, minimally invasive solutions for the Aesthetic and Vision markets. We are BeautyTech pioneers, empowering people by broadening the horizons of health.</Prose>
        </div>
      </Section>
      ...
      <Section>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 'var(--space-6)' }}>
          {PRODUCTS.map((p) => (
            <ProductCard key={p.name} name={p.name} market={p.market} onClick={(e) => { e.preventDefault(); onOpenProduct(p.sub); }} />
          ))}
        </div>
      </Section>
        ...
        <SplitLayout ratio="6fr 6fr" minHeight="420px">
          <SplitPanel tone="accent" align="space-between" pad="var(--space-9) var(--page-gutter)">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-5)' }}>
              <Headline size="title" mix="BRIGHT" tone="var(--accent-contrast)">A bright solution for dry eyes</Headline>
              <Button variant="inverse" size="sm">Learn more</Button>
            </div>
          </SplitPanel>
          <SplitPanel tone="page" align="space-between" pad="var(--space-9) var(--page-gutter)">
            ...
            <div style={{ display: 'flex', gap: 'var(--space-8)', paddingTop: 'var(--space-6)', borderTop: 'var(--border-width-hairline) solid var(--border-subtle)' }}>
              {([['first-aid-kit', 'Clinic'], ['user-circle', 'Customer'], ['sparkle', 'Treatment'], ['hand-heart', 'After-care']] as const).map(([n, l]) => (
                <div key={l} style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
                  <Icon name={n} size={28} />
