...
- **`scripts/migrations/`** holds the one-off mod itself — e.g.
  ...
  into `npm run build` or CI like
  `scripts/build-storybook-content.ts` is. This folder is
...
2. Write the mod as a script in `scripts/`, pulling shared logic from
   ...
   `scripts/build-storybook-content.ts` — check that file too for
   similar directory-scanning/marker-parsing patterns).
3. Run it, then verify: `npm run build` (or whatever the affected
   area's equivalent check is) to catch regressions the mod introduced.
4. If the mod touches how `scripts/build-storybook-content.ts` reads
