---
name: design-slides
description: Generate or edit slide reference cards for Lumenis, built on this system's real components, tokens, and shared deck-stage runtime. Use when the user asks for a deck, a slide, or a slide template.
---

# Design Slides

This skill covers the **generic reference cards** — `ui_kits/slides/*.html`,
the 7 standalone slide types with no product branding (`README.md` there is
their index).

Only when the user asks for it (a deck, a slide, a slide template) — never
generate this proactively while onboarding a product.

Per-product decks scaffolded under `products/<Name>/templates/` are not
supported — Claude Design's scanner only discovers `.dc.html` files under
the root `templates/` directory, never a nested `products/<Name>/templates/`
path, so anything built there is invisible to the design tool. There is
currently no working mechanism for a product-scoped deck; don't scaffold one.

## Building a great slide

`ui_kits/slides/README.md` is the index of the 7 slide types. This is how to
actually use them well — pick a density, don't overflow a slide, vary the
motif instead of repeating it, and never fall back to generic type or a
stock gradient.
...
look more dramatic. The generic `ui_kits/slides/*.html` cards have no
product motif to draw on — leave motif treatment out of them, or use a
neutral placeholder shape, rather than borrowing a specific product's.
...
content for — don't fabricate to fill the slot. The generic reference cards
are the one exception: their placeholder copy just needs to demonstrate the
slide type, not trace to a real product.
