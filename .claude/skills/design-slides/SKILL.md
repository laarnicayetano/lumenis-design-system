---
name: design-slides
description: Generate or edit slide decks and reference cards for Lumenis products, built on this system's real components, tokens, and shared deck-stage runtime. Use when the user asks for a deck, a slide, a slide template, or "a presentation for <Product>".
---

# Design Slides

Two artifacts live under this skill, and share the same design principles:

- **Product decks** — `products/<Name>/templates/Slides.dc.html`, built on
  the shared `deck-stage.js` runtime, scoped to one product's accent via
  `data-subbrand`.
- **Generic reference cards** — `ui_kits/slides/*.html`, the 7 standalone
  slide types with no product branding (`README.md` there is their index).

Only when the user asks for it (a deck, a slide, a slide template, "a
presentation for `<Product>`") — never generate this proactively while
onboarding a product.

## Building a great slide — principles for both
...
before starting — it shows every pattern above in context.
