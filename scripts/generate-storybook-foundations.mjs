// Generates one Storybook .mdx docs page per guideline card and per ui_kit
// (shared and per-product), so guidelines/, products/*/guidelines/, and
// ui_kits/ get a browsable mirror inside Storybook without becoming a
// second source of truth — each page just iframes the real, unmodified
// file. Output is pure build product (like the old dist/), gitignored, and
// regenerated by the `storybook`/`build-storybook` npm scripts before
// Storybook itself runs. Storybook is now the only public site (see
// .github/workflows/deploy-pages.yml) — this replaced scripts/build.mjs's
// guideline/ui_kit handling, which no longer exists.
//
// Interactive kits (ui_kits/corporate-website, products/OptiLIFT/ui_kit/
// website, products/OptiLIGHT/ui_kit/website) need their JSX precompiled to
// a bundle.js before they're viewable — the same esbuild step
// scripts/build.mjs used to do. That compiled output goes into
// stories/generated/ui-kit-bundles/<same relative path as the source>/.
//
// Important: .storybook/main.ts's staticDirs does NOT map the raw
// `ui_kits/corporate-website` or `products/<Name>/ui_kit/website` folders
// at all — only the compiled copy, at the same public path the raw folder
// would otherwise occupy. Overlaying a compiled copy *on top of* a raw
// mapping via two staticDirs entries at the same path was tried and
// doesn't work: Storybook's dev server resolves overlapping staticDirs
// first-match-wins, but `storybook build`'s static copy is last-copied-
// wins — no single entry order satisfies both. Excluding the raw folder
// from staticDirs entirely sidesteps the ambiguity: one unambiguous source
// per public path, identical in dev and build. Every other file each kit's
// JSX reaches via a relative path (product-local assets/, shared root
// assets/, etc.) still resolves, since the compiled copy preserves the
// exact same relative depth as the source and staticDirs maps each
// product's other subfolders (guidelines/, assets/, templates/) normally.
//
...
function iframeBlock(src, w, h, label) {
  return `<iframe
  src="${escapeAttr(src)}"
...
async function writeGenerated(relOutPath, content) {
...
async function generateProductGuidelines() {
...
function guidelineMdx(title, card, relSrc, publicSrc) {
...
Auto-generated from \`${relSrc}\` — do not edit this file directly, edit the
source guideline instead. Regenerated by \`scripts/generate-storybook-foundations.mjs\`.

${iframeBlock(publicSrc, card.w + 40, card.h + 40, card.name)}
`;
}

async function generateStaticUiKit(kit) {
...
directly. Regenerated by \`scripts/generate-storybook-foundations.mjs\`.
...
\`scripts/generate-storybook-foundations.mjs\`. Combined into one page,
...
// Compiles an interactive kit's JSX entry to a bundle.js (same esbuild
// options scripts/build.mjs used to use — classic transform + CDN-global
// React alias, since every kit page loads React from a <script> tag, not a
// bundled copy) and writes a modified copy of its index.html (babel-
// standalone placeholder swapped for `<script src="./bundle.js">`, exactly
// like the old buildUiKitHtml()) into stories/generated/ui-kit-bundles/,
...
// shared root assets/, etc.) keeps resolving once staticDirs serves this
// generated copy over the raw source at the same public path.
async function compileInteractiveKit({ dir, entry, kitTitle, kitSubtitle }) {
...
\`scripts/generate-storybook-foundations.mjs\` (same esbuild step the old
...
async function main() {
  ...
  console.log(
    `[generate-storybook-foundations] wrote ${guidelineCount} guideline page(s), ${kitCount} ui-kit page(s) to stories/generated/`,
  );
}
