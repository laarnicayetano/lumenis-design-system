#!/usr/bin/env node
// One-off code mod: converts components/**/*.specimen.tsx (React source,
// SSR'd at build time) into a hand-loadable *.card.html demo page — CDN
// React/ReactDOM/Babel scripts, _ds_bundle.js, and the specimen's own JSX
// transpiled live in-browser via <script type="text/babel">. Format matches
// input/forms/forms.card.html exactly. See .claude/skills/code-mods/SKILL.md.
...
function evalObjectLiteral(node: ts.ObjectLiteralExpression): Record<string, unknown> {
...
async function convertFile(file: string) {
  ...
  for (const stmt of sourceFile.statements) {
    if (ts.isImportDeclaration(stmt)) {
      ...
      if (init && ts.isObjectLiteralExpression(init)) card = evalObjectLiteral(init) as unknown as Card;
      ...
      // Local helper consts the render function closes over (e.g. NAMES
      // arrays) — keep at module scope, same position as the source.
