...
1. **The delivered logo SVGs are single-ink** (one flat fill per file: rose, black, or white) — there's no vector matching the two-tone black-wordmark-plus-rose-shine lockup shown throughout the print guidelines. `brand-logo.card.html` uses the monotone files as delivered rather than a fabricated composite.
2. **The print guidelines are internally inconsistent about "Soft Rose Beige.​"** Its primary-palette page labels `#FABCAD` (Pantone 691CP) "Stellar Soft Rose Beige (TriLift)" — the same value as triLIFT 2.0's Rose Gold — while its own sub-brand summary pages (RFX/XPL/QSW/NdY/SmoothGLO/PhotoFABULOUS) label `#DEAA9B` "Stellar Soft Rose Beige" with no TriLift note. This repo's `--lum-peach-beige` (`#DEAA9B`) matches the latter, already-wired value; the `#FABCAD` swatch was left alone rather than guessed at.
3. **Handpiece sub-brands (RFX, XPL, NdY, QSW) and treatment sub-brands (SmoothGLO, PhotoFABULOUS)** are documented in the print guidelines with their own logo lockups and confirmed color values, but weren't scaffolded as separate products/cards in this pass — flag as a natural follow-up if they need their own presence in this system.
4. Only the 3 vector logo files and 2 device renders were pulled in from a much larger supplied asset set (25+ device/handpiece renders across multiple wavelengths, a full campaign photoshoot, and stock model imagery); the rest remain in a local, gitignored `input/` drop folder.
