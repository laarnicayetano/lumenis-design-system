...
export function InsightCard({ title, topics = [], image, href = '#', style, ...rest }: InsightCardProps) {
  return (
    <article style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)', ...style }} {...rest}>
      ...
      {topics.length ? <Eyebrow style={{ color: 'var(--text-muted)' }}>{topics.join(' · ')}</Eyebrow> : null}
      <h3 style={{ margin: 0, fontFamily: 'var(--font-sans)', fontWeight: 'var(--weight-regular)', fontSize: 'var(--text-body)', lineHeight: 'var(--leading-subtitle)', textWrap: 'pretty' as React.CSSProperties['textWrap'] }}>{title}</h3>
      <TextLink href={href} caps size="caption">Read more</TextLink>
    </article>
  );
}
